...
class EnvHelper {
   ...
   public static function safeDistrict (string $district): string {
      return preg_replace('/[^0-9A-Za-z]/', '', $district);
   }

   public static function safeShow(string $show): string {
      $show = substr($show, 0, 3);
      return preg_replace('/[^dsw]/', '', $show);
   }
   ...
   public static function getEditableCounties(EnvFile $env): string {
      ...
      if ($env->get('isLocal') == "1")
         return $env->get('localCounties') ?? $allCounties;
